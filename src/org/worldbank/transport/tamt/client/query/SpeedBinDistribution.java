...
public class SpeedBinDistribution extends Composite {

	private static SpeedBinDistributionUiBinder uiBinder = GWT
			.create(SpeedBinDistributionUiBinder.class);

	interface SpeedBinDistributionUiBinder extends UiBinder<Widget, SpeedBinDistribution> {
	}
	...
	public SpeedBinDistribution(HandlerManager eventBus) {
	...
	void onClickDownloadReport(ClickEvent e){
		...
		if( currentTagDetails != null)
		{
			...
			String url = "/download/speedbin/speeddistribution?tagid=" + currentTagDetails.getId();
	...
	void onClickGenerateReports(ClickEvent e) {
		...
		String confirm = 	"Create speed distribution report? This will delete\n" +
							"subsequent reports. You will need to recreate them\n" +
							"after this delete operation is completed.";
	...
	private void generateReports()
	{
		...
		dialog.setWidget(new HTML("Generating speed distribution report for all tags. <br/>This may take a few minutes"));
		dialog.setAutoHideEnabled(false);
		dialog.setGlassEnabled(true);
		dialog.center();
		dialog.show();
		
		speedDistributionReportService.createSpeedDistributionReport(new AsyncCallback<Void>() {
	...
	private void bind() {
		...
		eventBus.addHandler(TAMTResizeEvent.TYPE, new TAMTResizeEventHandler() {
			...
			public void onTAMTResize(TAMTResizeEvent event) {
				GWT.log("SIZE: SpeedBinDistribution width: " + event.width);
				...
				if( h > -1)
				{
					...
					GWT.log("SIZE: SpeedBinDistribution adjusted height: " + height);
					...
					GWT.log("SIZE: SpeedBinDistribution adjusted width: " + width);
	...
	protected void getSpeedDistributionReport(TagDetails tagDetails) {
		...
		speedDistributionReportService.getSpeedDistributionReport(tagDetails, new AsyncCallback<SpeedDistributionReport>() {
			
			@Override
			public void onSuccess(SpeedDistributionReport result) {
				// TODO Auto-generated method stub
				GWT.log("getSpeedDistributionReport result:" + result);
	...
	protected void renderReport(SpeedDistributionReport result) {
		...
		for (Iterator iterator = reportValues.iterator(); iterator.hasNext();) {
			...
			for (Iterator iter = thisRow.iterator(); iter.hasNext();) {
				...
				{
					l.setStyleName(style.hour());
				} else if (column == 3)
				{
					l.setStyleName(style.cellHeaderInteger());
				} else {
