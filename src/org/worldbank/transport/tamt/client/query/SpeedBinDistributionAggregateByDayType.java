...
public class SpeedBinDistributionAggregateByDayType extends Composite {

	private static SpeedBinDistributionAggregateByDayTypeUiBinder uiBinder = GWT
			.create(SpeedBinDistributionAggregateByDayTypeUiBinder.class);

	interface SpeedBinDistributionAggregateByDayTypeUiBinder extends UiBinder<Widget, SpeedBinDistributionAggregateByDayType> {
	}
	...
	public SpeedBinDistributionAggregateByDayType(HandlerManager eventBus) {
	...
	void onClickGenerateReports(ClickEvent e) {
		...
		String confirm = 	"Create aggregate by day type report?\n" +
	...
	private void generateReports()
	{
		...
		speedDistributionReportService.createSpeedDistributionAggregateByDayTypeReport(new AsyncCallback<Void>() {
			...
			public void onSuccess(Void result) {
				dialog.hide();
				resetScreen();
				Window.alert("Aggregate by day type reports have been generated");
			}
		});
	...
	private void bind() {
		...
		eventBus.addHandler(TAMTResizeEvent.TYPE, new TAMTResizeEventHandler() {
			...
			public void onTAMTResize(TAMTResizeEvent event) {
				GWT.log("SIZE: SpeedBinDistributionAggregateByDayType width: " + event.width);
				int h = event.height - 225; // account for other query module UI
				int reportH = h - 20;
				int w = event.width - 283; // x - 696 = 564; .: x = 564 + 696 = 1260; 1260 - x = 413
				GWT.log("SIZE: SpeedBinDistributionAggregateByDayType adjusted width: " + w);
				if( h > -1)
				{
					String height = Integer.toString(h) + "px";
					GWT.log("SIZE: SpeedBinDistributionAggregateByDayType adjusted height: " + height);
					...
					GWT.log("SIZE: SpeedBinDistributionAggregateByDayType adjusted width: " + width);
	...
	protected void getSpeedDistributionAggregateByDayTypeReport(TagDetails tagDetails) {
		...
		speedDistributionReportService.getSpeedDistributionAggregateByDayTypeReport(tagDetails, new AsyncCallback<SpeedDistributionAggregateByDayTypeReport>() {

			public void onSuccess(SpeedDistributionAggregateByDayTypeReport result) {
				// TODO Auto-generated method stub
				GWT.log("getSpeedDistributionAggregateByDayTypeReport result:" + result);
	...
	protected void renderReport(SpeedDistributionAggregateByDayTypeReport result) {
