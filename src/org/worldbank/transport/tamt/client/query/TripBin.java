...
public class TripBin extends Composite {

	private static TripBinUiBinder uiBinder = GWT
			.create(TripBinUiBinder.class);

	interface TripBinUiBinder extends UiBinder<Widget, TripBin> {
	}
	...
	public TripBin(HandlerManager eventBus) {
	...
	void onClickDownloadReport(ClickEvent e){
		...
		if( currentStudyRegion != null)
		{
			...
			String url = "/download/tripbin?regionid=" + currentStudyRegion.getId();
	...
	public void getTripStatisticsReport()
	{
		reportTable.removeAllRows();
		reportTable.clear();
		...
		tripStatisticsReportService.getTripStatisticsReport(new AsyncCallback<TripStatisticsReport>() {
			...
			public void onSuccess(TripStatisticsReport result) {
				GWT.log("getTripStatisticsReport result:" + result);
				if( result == null)
				{
